...
int main(void)
{
  ...
  MPI_Type_contiguous(local_n, MPI_DOUBLE, &VECTOR_TYPE);
...
void Read_vector(int my_rank, int n, int local_n, double* vec, double* local_vec)
{
  ...
  MPI_Scatter(vec, 1, VECTOR_TYPE, local_vec, local_n, MPI_DOUBLE, 0, MPI_COMM_WORLD);
}
...
void Print_vector(double local_vec[], int local_n, int n, char title[], int my_rank, MPI_Comm comm) {
  ...
  MPI_Gather(local_vec, local_n, MPI_DOUBLE, a, 1, VECTOR_TYPE, 0, comm);
